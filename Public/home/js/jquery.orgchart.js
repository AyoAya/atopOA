...
(function(factory) {
  ...
  function showSiblings($node, direction) {
    ...
    $siblings.find('.node:visible').addClass('slide').removeClass('slide-left slide-right').eq(-1).one('transitionend', function() {
      ...
      if (isInAction($node)) {
        switchHorizontalArrow($node);
        $node.children('.topEdge').removeClass('icon-chevron-up').addClass('icon-chevron-down');
      }
    });
  }
  ...
  function switchVerticalArrow($arrow) {
    $arrow.toggleClass('icon-chevron-up').toggleClass('icon-chevron-down');
  }
    ...
    if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' || $node.closest('.nodes').data('siblingsLoaded'))) {
      ...
      if ($prevSib.length) {
        if ($prevSib.is('.hidden')) {
          $node.children('.leftEdge').addClass('icon-chevron-left').removeClass('icon-chevron-right');
        } else {
          $node.children('.leftEdge').addClass('icon-chevron-right').removeClass('icon-chevron-left');
        }
      }
      var $nextSib = $node.closest('table').parent().next();
      if ($nextSib.length) {
        if ($nextSib.is('.hidden')) {
          $node.children('.rightEdge').addClass('icon-chevron-right').removeClass('icon-chevron-left');
        } else {
          $node.children('.rightEdge').addClass('icon-chevron-left').removeClass('icon-chevron-right');
        }
      }
    } else {
      var $sibs = $node.closest('table').parent().siblings();
      var sibsVisible = $sibs.length ? !$sibs.is('.hidden') : false;
      $node.children('.leftEdge').toggleClass('icon-chevron-right', sibsVisible).toggleClass('icon-chevron-left', !sibsVisible);
      $node.children('.rightEdge').toggleClass('icon-chevron-left', sibsVisible).toggleClass('icon-chevron-right', !sibsVisible);
    }
  }
    ...
    $nodeDiv.on('mouseenter mouseleave', function(event) {
      ...
      if (event.type === 'mouseenter') {
        if ($topEdge.length) {
          ...
          $topEdge.toggleClass('icon-chevron-up', !flag).toggleClass('icon-chevron-down', flag);
        }
        if ($bottomEdge.length) {
          flag = getNodeState($node, 'children').visible;
          $bottomEdge.toggleClass('icon-chevron-down', !flag).toggleClass('icon-chevron-up', flag);
        }
        if ($leftEdge.length) {
          switchHorizontalArrow($node);
        }
      } else {
        $node.children('.edge').removeClass('icon-chevron-up icon-chevron-down icon-chevron-right icon-chevron-left');
      }
    });
