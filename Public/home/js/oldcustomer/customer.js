...
$(function(){
	...
	$('#form').validate({
		...
		submitHandler : function(form){
			...
			$(form).ajaxSubmit({
				url : ThinkPHP['AJAX'] + '/Customer/addCustomer',
				type : 'POST',
				data : {
					cc_time : $('#cc_time').val(),
	...
	/**
	 * uploader : ThinkPHP['AJAX'] + '/Customer/uploadFile',
	...
	$('#processingRecords').validate({
		...
		submitHandler : function(form){
			...
			$(form).ajaxSubmit({
				url : ThinkPHP['AJAX'] + '/Customer/addComplaintLog',
				type : 'POST',
				data : {
					cc_id : $('#cc_id').val(),
	...
	$('#file-preview').on('click','.remove-file',function(){
		...
		$.post(ThinkPHP['AJAX'] + '/Customer/unlinkPic',{source:source},function(data){
			if(data=='true'){
				//图像删除后移除该DOM元素
				_this.parent().remove();
				//重新计算文件数量并删除对应数组内的元素
