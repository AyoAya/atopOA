...
$(function(){
    ...
    layui.use(['form','layer'], function(){
        ...
        form.on('select(vendor)', function( data ){
            var vendor_name = $(data.elem).find('option[value='+ data.value +']').text();
            $('#vendor_name').val(vendor_name);
        });

        form.on('submit(compatibility)', function( data ){
            //console.log(data);
            $.ajax({
                ...
                success : function( response ){
                    if( response.flag > 0 ){
                        ...
                        setTimeout(function(){
                            location.href = 'http://'+ThinkPHP['HTTP_HOST']+'/Compatibility';
                        },2000);
                    }else{
    ...
    $(document).on('click', '.filter-item', function(){
        ...
        $.ajax({
            url : ThinkPHP['AJAX'] + '/Compatibility/productFilter',
            type : 'POST',
            data : {
                filter : {
