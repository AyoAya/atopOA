...
<!-- 网站标题 -->
	...
	<link rel="stylesheet" href="/Public/Home/css/system.css">
<!-- 引入css文件 -->
	...
	<script src="/Public/Home/js/system.js"></script>
<!-- 引入js文件 -->
...
<body>
	...
	<div class="col-lg-6 col-lg-offset-3 system-container">
		<div class="page-header">
			<h2>验证码</h2>
		</div>
		<div class="config-verify-box">
			<h4>登录验证码<span class="verify-switch <?php if((C("LOGIN_VERIFY")) == "1"): ?>active<?php endif; ?>" value="0" key="LOGIN_VERIFY" flag="verify"></span></h4>
			<div class="config-verify-info <?php if((C("LOGIN_VERIFY")) == "0"): ?>sr-only<?php endif; ?>">
				<p>是否使用随机背景<span class="verify-switch verify-config-switch <?php if((C("VERIFY_USEIMGBG")) == "1"): ?>active<?php endif; ?>" value="0" key="VERIFY_USEIMGBG"></span></p>
				<p>是否使用混淆曲线<span class="verify-switch verify-config-switch <?php if((C("VERIFY_USECURVE")) == "1"): ?>active<?php endif; ?>" value="0" key="VERIFY_USECURVE"></span></p>
				<p>是否添加混淆杂点<span class="verify-switch verify-config-switch <?php if((C("VERIFY_USENOISE")) == "1"): ?>active<?php endif; ?>" value="0" key="VERIFY_USENOISE"></span></p>
			</div>
		</div>
		<div class="page-header">
			<h2>上传文件</h2>
		</div>
		<div class="config-upload-box">
			<div class="config-upload-info">
				<form role="form" class="upload-form-config-box" id="uploadConfigForm">
					<div class="form-group upload-config-input">
						<label>文件上传大小限制</label>
						<div class="input-group">
							<input type="text" class="form-control" value="<?php echo (C("UPLOAD_FILESIZELIMIT")); ?>" name="FileSizeLimit">
							<div class="input-group-addon">M</div>
						</div>
						<div class="help-block"></div>
					</div>
					<div class="form-group">
						<label>文件上传最大数量</label>
						<div class="input-group upload-config-input">
							<input type="text" class="form-control" value="<?php echo (C("UPLOAD_UPLOADLIMIT")); ?>" name="UploadLimit">
							<div class="input-group-addon">个</div>
						</div>
						<div class="help-block"></div>
					</div>
					<div class="form-group">
						<label>文件上传格式限制</label>
						<input type="text" class="form-control upload-config-input-exts" value="<?php echo (C("UPLOAD_FILETYPEEXTS")); ?>" name="FileTypeExts">
						<div class="help-block"></div>
					</div>
					<div class="form-group">
						<input type="submit" class="btn btn-primary btn-lg btn-submit" value="保存">
					</div>
				</form>
			</div>
		</div>
		...
		<div class="config-page-box">
			<h4>分页状态信息<span class="verify-switch <?php if((C("PAGE_STATUS_INFO")) == "1"): ?>active<?php endif; ?>" value="0" key="PAGE_STATUS_INFO" flag="page"></span></h4>
			<form role="form" id="pageConfigForm">
				<div class="input-group">
					<span class="input-group-addon">每页显示</span>
					<input type="text" class="form-control" name="pageLimitSize" value="<?php echo (C("LIMIT_SIZE")); ?>">
		...
		<div class="config-page-box">
			<form role="form" id="cookieConfigForm">
				<div class="input-group input-group-cookie">
					<input type="text" class="form-control" name="cookieKey" value="<?php echo (C("MD5_KEY")); ?>" data-toggle="tooltip" data-placement="left" title="cookie加密唯一标识" onMouseOver="$(this).tooltip('show')">
					<span class="input-group-btn">
						<button type="submit" class="btn btn-default">确定</button>
					</span>
				</div>
