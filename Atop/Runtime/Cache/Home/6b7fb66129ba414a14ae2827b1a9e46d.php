...
<body>
    ...
    <?php if(($details["rma_state"]) == "N"): ?>
        <?php if(($details["version"]) == "new"): ?><div class="btn-group pull-right btn-group-edit" role="group">
                <?php if(in_array(($_SESSION['user']['id']), is_array($details["operation_person"])?$details["operation_person"]:explode(',',$details["operation_person"]))): ?>
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#customer-rma-modal">
	...
	<!-- 销售确认是否转为RMA[展开] -->
	<!--<?php if(($details["sale_audit"]) == "1"): if(($face["account"]) == $details["salesperson"]): ?><div class="layui-collapse animated fadeInRight">
				<div class="layui-colla-item">
					<h4 class="layui-colla-title layui-colla-title-rewrite">是否转为RMA</h4>
					<div class="layui-colla-content layui-show">
						&lt;!&ndash; 加载到layer弹出层 &ndash;&gt;
						<form class="layui-form rma-form">
							<input type="hidden" name="recorder" value="<?php echo ($face["account"]); ?>">
							<input type="hidden" name="uid" value="<?php echo ($_SESSION['user']['id']); ?>">
							<input type="hidden" name="cc_id" value="<?php echo ($details["id"]); ?>">
							<div class="layui-inline layui-form-item-rewrite">
								<label class="layui-form-label">RMA</label>
								<div class="layui-input-block">
									<input type="radio" name="is_rma" value="1" title="是" checked lay-filter="is_rma">
									<input type="radio" name="is_rma" value="0" title="否" lay-filter="is_rma">
								</div>
							</div>
							<div class="layui-inline">
								<label class="layui-form-label">QA</label>
								<div class="layui-input-block">
									<select name="qa_responsible">
										<?php if(is_array($QAlist)): $i = 0; $__LIST__ = $QAlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><option value="<?php echo ($value["id"]); ?>"><?php echo ($value["nickname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							</div>
							<div class="layui-inline">
								...
								<div class="layui-input-block">
									<input type="text" readonly name="log_date" class="layui-input processing-time">
								</div>
							</div>
							<div class="layui-form-item layui-form-text layui-form-text-rewrite">
								<textarea name="log_content" placeholder="备注信息" class="layui-textarea"></textarea>
							</div>
							<div class="layui-form-item">
								<button lay-submit lay-filter="rma" class="layui-btn pull-left rma-save-btn">保存</button>
								<div class="pull-left">
